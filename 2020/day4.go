		...
		for _, part := range parts {
			...
			if kv[0] == "hgt" {
				if strings.Contains(kv[1], "cm") {
					kv[1] = strings.TrimRight(kv[1], "cm")
					val := strToInt(kv[1])
					if val < 150 || val > 193 {
						contains = false
						break
					...
					kv[1] = strings.TrimRight(kv[1], "in")
					val := strToInt(kv[1])
					if val < 59 || val > 76 {
						contains = false
						break
			...
			if kv[0] == "hcl" {
				...
				kv[1] = kv[1][1:]
				res := regexReplaceAll(`[a-z0-9]*`, kv[1], "")
				if string(res) != "" {
					contains = false
				}
			}
			...
			if kv[0] == "ecl" {
				if !inArray(kv[1], []string{"amb", "blu", "brn", "gry", "grn", "hzl", "oth"}) {
					contains = false
				}
			}

			if kv[0] == "pid" {
				res := regexReplaceAll(`[0-9]{9}`, kv[1], "")
				if string(res) != "" {
					contains = false
				}
			}
