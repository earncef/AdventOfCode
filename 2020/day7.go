...
func day7() int {
	input := strings.TrimSpace(day7input)
	input = strings.ReplaceAll(input, "bags", "")
	inputArr := strings.Split(input, "\n")
	canContain := map[string]bool{}
	wantColors := []string{"shiny gold"}
	colorMaps := map[string]string{}
	for _, line := range inputArr {
		parts := strings.Split(line, " contain ")
		colorMaps[strings.TrimSpace(parts[0])] = strings.TrimSpace(parts[1])
	}

	for len(wantColors) > 0 {
		color := strings.TrimSpace(wantColors[0])
		wantColors = wantColors[1:]
		for key, val := range colorMaps {
			if strings.Contains(val, color) {
				canContain[key] = true
				wantColors = append(wantColors, key)
			}
		}
	}
...
func day7part2() int {
	...
	inputArr := strings.Split(input, "\n")
	colorMaps := map[string]map[string]int{}
	for _, line := range inputArr {
		parts := strings.Split(line, " contain ")
		types := strings.Split(strings.TrimSpace(parts[1]), ", ")
		key := strings.TrimSpace(parts[0])
		colorMaps[key] = map[string]int{}
		for _, t := range types {
			p := strings.SplitN(t, " ", 2)
			colorMaps[key][strings.TrimSpace(p[1])] = strToInt(p[0])
		}
	}

	return countBags("shiny gold", colorMaps)
}
