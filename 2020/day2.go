...
func day2() int {
    input := strings.TrimSpace(day2input)
    inputArr := strings.Split(input, "\n")
    count := 0
    for _, line := range inputArr {
        parts := strings.Split(line, ":")
        rule := parts[0]
        inp := strings.TrimSpace(parts[1])

        ch := rule[len(rule)-1:]
        rule = strings.TrimSpace(rule[:len(rule)-1])
        ruleParts := strings.Split(rule, "-")
        min, _ := strconv.Atoi(ruleParts[0])
        max, _ := strconv.Atoi(ruleParts[1])
        charCount := 0
        for _, c := range inp {
            if string(c) == ch {
                charCount++
...
func day2part2() int {
    ...
    for _, line := range inputArr {
        ...
        rule = strings.TrimSpace(rule[:len(rule)-1])
        ruleParts := strings.Split(rule, "-")
        pos1, _ := strconv.Atoi(ruleParts[0])
        pos2, _ := strconv.Atoi(ruleParts[1])
        if (inp[pos1-1] == ch && inp[pos2-1] != ch) || (inp[pos1-1] != ch && inp[pos2-1] == ch) {
            count++
        }
    }
