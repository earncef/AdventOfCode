...
func day11() int {
	// input := strings.TrimSpace(day11inputTest)
	input := strings.TrimSpace(day11inputReal)
	...
	for {
		...
		for i, line := range inputArr {
			...
			for j, s := range line {
				...
				occupied := getOccupiedNumAdjacentSeats(inputArr, i, j)
				if seat == "L" {
					if occupied == 0 {
						outputLine += "#"
						finalOccupied++
	...
	for {
		...
		for i, line := range inputArr {
			...
			for j, s := range line {
				...
				occupied := getFirstVisibleOccupiedNumAdjacentSeats(inputArr, i, j)
				if seat == "L" {
					if occupied == 0 {
						outputLine += "#"
						finalOccupied++
...
func getFirstVisibleOccupiedNumAdjacentSeats(lines []string, i, j int) int {
	occupied := 0
	...
	for {
		...
		seat := string(lines[i-count][j-count])
		if seat == "#" {
			occupied++
			break
		} else if seat == "L" {
	...
	count = 1
	for {
		if i-count < 0 || j+count >= len(lines[0]) {
			break
		}
		seat := string(lines[i-count][j+count])
		if seat == "#" {
	...
	count = 1
	for {
		if i+count >= len(lines) || j-count < 0 {
			break
		}
		seat := string(lines[i+count][j-count])
		if seat == "#" {
	...
	count = 1
	for {
		if i+count >= len(lines) || j+count >= len(lines[0]) {
			break
		}
		seat := string(lines[i+count][j+count])
		if seat == "#" {
...
func getOccupiedNumAdjacentSeats(lines []string, i, j int) int {
	occupied := 0
	if i-1 >= 0 {
		if j-1 >= 0 {
		...
		if string(lines[i-1][j]) == "#" {
			occupied++
		}
		if j+1 < len(lines[0]) && string(lines[i-1][j+1]) == "#" {
			occupied++
		}
	}

	if j-1 >= 0 && string(lines[i][j-1]) == "#" {
		occupied++
	}

	if j+1 < len(lines[0]) && string(lines[i][j+1]) == "#" {
		occupied++
	}

	if i+1 < len(lines) {
		if j-1 >= 0 && string(lines[i+1][j-1]) == "#" {
			occupied++
		}
		if string(lines[i+1][j]) == "#" {
			occupied++
